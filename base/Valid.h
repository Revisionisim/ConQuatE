...
void validHead(REAL *con) {
    ...
    if (l_filter_s < 10) l_valid_filter_hit10 += 1;
    if (l_filter_s < 3) l_valid_filter_hit3 += 1;
    if (l_filter_s <= 1) l_valid_filter_hit1 += 1;
    lastValidHead ++;
...
void validTail(REAL *con) {
    ...
    if (r_filter_s < 10) r_valid_filter_hit10 += 1;
    if (r_filter_s < 3) r_valid_filter_hit3 += 1;
    if (r_filter_s <= 1) r_valid_filter_hit1 += 1;
    lastValidTail ++;
//    printf("tail: r_valid_filter_tot = %f | r_filter_hit10 = %f\n", r_valid_filter_tot, r_valid_filter_tot / lastValidTail);
}

REAL validHit1,validHit3,validHit10 = 0;
extern "C"
REAL  getValidHit() {
    // l_valid_filter_tot /= validTotal;
    // r_valid_filter_tot /= validTotal;
    Hit1 = (l_valid_filter_hit1 + r_valid_filter_hit1) / (2*validTotal);
    Hit3 = (l_valid_filter_hit3 + r_valid_filter_hit3) / (2*validTotal);
    Hit10 = (l_valid_filter_hit10 + r_valid_filter_hit10) / (2*validTotal);
    Hit[3]={Hit1,Hit3,Hit10}
   // printf("result: %f\n", validHit10);
    return Hit[3];
}
